...
Trivi використовує AI від Google, щоб давати відповіді. Для цього потрібен «ключ» — довгий рядок символів, який Google видає безкоштовно. Без банківської картки. Безкоштовно для звичайного користування.

1. Відкрийте `aistudio.google.com` у браузері.
2. Увійдіть будь-яким Google-акаунтом.
3. Натисніть **Get API Key** у лівій панелі.
4. Натисніть **Create API Key**.
5. Скопіюйте ключ (починається з `AIza…`). Не закривайте вкладку — за хвилину вставимо ключ.

![Google AI Studio — ключ](screenshots/google-apikey.png)

---

## Крок 2 — Відкрийте Trivi та вставте ключ

1. Відкрийте посилання на Trivi, яке ви отримали разом із покупкою.
2. Клікніть значок шестерні (праворуч угорі), щоб відкрити Settings.
3. Вставте свій ключ у поле **Gemini API Key**.
4. Натисніть **Save**.

![Settings — ключі](screenshots/settings-api-keys.png)

Ось і все налаштування. Тепер Trivi використовує цей ключ кожного разу, коли ви щось запитуєте.
...
- **«Invalid API key» або взагалі немає відповіді.** Відкрийте Settings, перевірте, що ключ починається з `AIza…` і немає зайвих пробілів. Збережіть ще раз.
